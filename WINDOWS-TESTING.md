...
- [ ] **Terminal.Gui bump `2.4.17-develop.6` → `2.4.18-develop.6`.** Small bump (6 upstream commits): adds an editable `NumericUpDown` mode (unused by this app — no numeric input fields) and fixes `SynchronizationContext` not being correctly implemented in v2 (upstream #5579/#5588). This app already marshals background-thread UI updates via `App?.Invoke(...)` rather than relying on `SynchronizationContext`, so the fix shouldn't change observable behavior here, but do a normal pass since it wasn't previously exercised. Mock-backend smoke on Linux (main list, detail panel, Help dialog, Search/Installed tab switch, 109/109 unit tests) showed no regressions.
